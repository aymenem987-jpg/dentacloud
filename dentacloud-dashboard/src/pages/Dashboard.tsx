...
function PopupPlans({ onClose }: { onClose: () => void }) {
  return (
    <div onClick={(e: React.MouseEvent<HTMLDivElement>) => e.target === e.currentTarget && onClose()} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div style={{ background: 'rgba(13,31,28,0.98)', border: '1px solid rgba(18,160,143,0.2)', borderRadius: '20px', padding: 'clamp(1.5rem, 2.5vw, 2.5rem)', width: '100%', maxWidth: 'min(900px, 92vw)', maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.2rem' }}>
          {plans.map((plan, i) => (
            <div key={i} style={{ background: plan.populaire ? 'linear-gradient(135deg, rgba(200,151,58,0.15), rgba(13,31,28,0.9))' : 'rgba(19,36,32,0.8)', border: `1px solid ${plan.populaire ? '#C8973A' : 'rgba(18,160,143,0.15)'}`, borderRadius: '16px', padding: 'clamp(1.5rem, 2vw, 2rem)', position: 'relative', boxShadow: plan.populaire ? '0 0 0 1px #C8973A, 0 20px 40px rgba(200,151,58,0.15)' : 'none' }}>
              {plan.populaire && <div style={{ position: 'absolute', top: '-12px', left: '50%', transform: 'translateX(-50%)', background: 'linear-gradient(135deg, #C8973A, #E8B55A)', color: '#fff', fontSize: '0.7rem', fontWeight: 600, padding: '0.25rem 0.8rem', borderRadius: '100px', whiteSpace: 'nowrap' }}>⭐ LE PLUS POPULAIRE</div>}
              <div style={{ fontSize: '0.78rem', fontWeight: 600, color: '#8BBDB5', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.5rem' }}>{plan.nom}</div>
              <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(2rem, 3vw, 2.8rem)', fontWeight: 700, lineHeight: 1, marginBottom: '0.3rem' }}>
                <sup style={{ fontSize: '1.05rem', verticalAlign: 'top', marginTop: '0.5rem', display: 'inline-block' }}>DZD</sup>{plan.prix}<span style={{ fontSize: '1.05rem', fontWeight: 400, color: '#8BBDB5' }}>/mois</span>
              </div>
              <div style={{ fontSize: '0.85rem', color: '#8BBDB5', marginBottom: '1.2rem' }}>{plan.desc}</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem', marginBottom: '1.5rem' }}>
                {plan.features.map((f, j) => (
                  <div key={j} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem', color: '#8BBDB5' }}>
                    <span style={{ color: plan.couleur }}>✓</span>{f}
                  </div>
                ))}
              </div>
              <button onClick={() => { const msg = `Bonjour DentaCloud ! Je voudrais souscrire au plan ${plan.nom} à ${plan.prix} DZD/mois.`; window.open(`https://wa.me/213775538234?text=${encodeURIComponent(msg)}`, '_blank') }} style={{ width: '100%', padding: '0.9rem', background: plan.populaire ? 'linear-gradient(135deg, #C8973A, #E8B55A)' : 'linear-gradient(135deg, #0A7C6E, #12A08F)', color: '#fff', border: 'none', borderRadius: '8px', fontFamily: "'DM Sans', sans-serif", fontSize: '0.9rem', fontWeight: 600, cursor: 'pointer' }}>
                Souscrire via WhatsApp →
              </button>
            </div>
          ))}
        </div>
        <p style={{ textAlign: 'center', fontSize: '0.8rem', color: '#8BBDB5', marginTop: '1.5rem' }}>✓ Paiement sécurisé · ✓ Support inclus · ✓ Données conservées</p>
      </div>
    </div>
  )
}
  ...
  const renderBanniere = () => {
    ...
    if (joursRestants <= 0) return (
      <div style={{ background: 'linear-gradient(135deg, rgba(229,115,115,0.15), rgba(229,115,115,0.05))', border: '1px solid rgba(229,115,115,0.3)', borderRadius: '12px', padding: 'clamp(1rem, 1.3vw, 1.4rem) clamp(1.2rem, 1.8vw, 2rem)', marginBottom: '1.8rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
        <div><div style={{ fontWeight: 600, color: '#E57373', marginBottom: '0.2rem', fontSize: 'clamp(0.95rem, 1.1vw, 1.1rem)' }}>⚠️ Votre essai gratuit est expiré</div><div style={{ fontSize: 'clamp(0.82rem, 0.95vw, 0.95rem)', color: '#8BBDB5' }}>Souscrivez maintenant pour continuer</div></div>
        <button onClick={() => setShowPlans(true)} style={{ background: 'linear-gradient(135deg, #0A7C6E, #12A08F)', color: '#fff', border: 'none', borderRadius: '8px', padding: '0.75rem 1.6rem', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 'clamp(0.85rem, 1vw, 1rem)' }}>🚀 Souscrire maintenant</button>
      </div>
    )
    if (joursRestants <= 7) return (
      <div style={{ background: 'linear-gradient(135deg, rgba(200,151,58,0.15), rgba(200,151,58,0.05))', border: '1px solid rgba(200,151,58,0.3)', borderRadius: '12px', padding: 'clamp(1rem, 1.3vw, 1.4rem) clamp(1.2rem, 1.8vw, 2rem)', marginBottom: '1.8rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
        <div><div style={{ fontWeight: 600, color: '#E8B55A', marginBottom: '0.2rem', fontSize: 'clamp(0.95rem, 1.1vw, 1.1rem)' }}>⏳ Plus que {joursRestants} jour{joursRestants > 1 ? 's' : ''} d'essai gratuit</div><div style={{ fontSize: 'clamp(0.82rem, 0.95vw, 0.95rem)', color: '#8BBDB5' }}>Souscrivez avant expiration</div></div>
        <button onClick={() => setShowPlans(true)} style={{ background: 'linear-gradient(135deg, #C8973A, #E8B55A)', color: '#fff', border: 'none', borderRadius: '8px', padding: '0.75rem 1.6rem', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 'clamp(0.85rem, 1vw, 1rem)' }}>Souscrire →</button>
      </div>
    )
    return (
      <div style={{ background: 'linear-gradient(135deg, rgba(18,160,143,0.1), rgba(18,160,143,0.05))', border: '1px solid rgba(18,160,143,0.2)', borderRadius: '12px', padding: 'clamp(0.85rem, 1vw, 1.1rem) clamp(1.2rem, 1.8vw, 2rem)', marginBottom: '1.8rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.5rem' }}>
        <div style={{ fontSize: 'clamp(0.82rem, 0.95vw, 0.95rem)', color: '#8BBDB5' }}>🎉 Essai gratuit — <strong style={{ color: '#12A08F' }}>{joursRestants} jours restants</strong></div>
        <button onClick={() => setShowPlans(true)} style={{ background: 'transparent', color: '#12A08F', border: '1px solid rgba(18,160,143,0.3)', borderRadius: '6px', padding: '0.35rem 0.9rem', cursor: 'pointer', fontSize: 'clamp(0.78rem, 0.9vw, 0.9rem)', fontFamily: "'DM Sans', sans-serif" }}>Voir les plans →</button>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '1.8rem' }}>
        <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.7rem, 2.4vw, 2.6rem)', fontWeight: 700, marginBottom: '0.3rem' }}>Bonjour, {userName} 👋</h1>
        <p style={{ color: '#8BBDB5', fontSize: 'clamp(0.85rem, 1vw, 1.05rem)' }}>Voici un aperçu de votre activité</p>
      </div>

      {renderBanniere()}

      {/* KPI grid: la taille minimale des cartes augmente avec l'écran grâce a clamp(),
          donc sur un écran 32 pouces on a moins de colonnes mais des cartes plus grandes
          au lieu d'avoir 6 petites cartes minuscules alignées */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(clamp(160px, 14vw, 230px), 1fr))', gap: 'clamp(1rem, 1.3vw, 1.5rem)', marginBottom: '1.8rem' }}>
        {kpis.map((k, i) => (
          <div key={i} style={{ ...card, transition: 'transform 0.2s' }} onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-3px)'} onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.transform = 'translateY(0)'}>
            ...
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(2rem, 2.6vw, 2.8rem)', fontWeight: 700, lineHeight: 1, marginBottom: '0.4rem' }}>{loading ? '...' : k.value}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(clamp(280px, 28vw, 420px), 1fr))', gap: 'clamp(1rem, 1.3vw, 1.5rem)', marginBottom: '1.8rem' }}>
        <div style={card}>
          <div style={{ fontSize: 'clamp(0.72rem, 0.85vw, 0.9rem)', fontWeight: 600, color: '#8BBDB5', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '1.2rem' }}>Activité — 7 derniers jours</div>
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: '6px' }}>{jours.map((j, i) => (<div key={i} style={{ flex: 1, textAlign: 'center', fontSize: 'clamp(0.6rem, 0.7vw, 0.75rem)', color: i === 5 ? '#C8973A' : '#8BBDB5' }}>{j}</div>))}</div>
        </div>

        <div style={card}>
          <div style={{ fontSize: 'clamp(0.72rem, 0.85vw, 0.9rem)', fontWeight: 600, color: '#8BBDB5', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '1.2rem' }}>Types de soins</div>
          {[{ label: 'Détartrage', pct: 40, color: '#12A08F' }, { label: 'Carie / Obturations', pct: 30, color: '#C8973A' }, { label: 'Orthodontie', pct: 20, color: '#7C3AED' }, { label: 'Autres', pct: 10, color: '#8BBDB5' }].map((s, i) => (
            <div key={i} style={{ marginBottom: '0.9rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'clamp(0.72rem, 0.85vw, 0.9rem)', marginBottom: '0.3rem' }}><span>{s.label}</span><span style={{ color: s.color, fontWeight: 600 }}>{s.pct}%</span></div>
              <div style={{ height: '6px', background: 'rgba(255,255,255,0.06)', borderRadius: '100px' }}><div style={{ height: '100%', width: `${s.pct}%`, background: s.color, borderRadius: '100px' }} /></div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ ...card, padding: 0 }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <div style={{ minWidth: '500px' }}>
            ...
            : rdvList.map((rdv, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '0.6rem', padding: '0.7rem clamp(1.2rem, 1.5vw, 1.6rem)', borderTop: '1px solid rgba(18,160,143,0.05)', fontSize: 'clamp(0.78rem, 0.9vw, 0.95rem)', transition: 'background 0.15s' }} onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.background = 'rgba(18,160,143,0.04)'} onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.background = 'transparent'}>
                ...
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '0.2rem 0.6rem', borderRadius: '100px', fontSize: 'clamp(0.65rem, 0.75vw, 0.8rem)', fontWeight: 500, background: rdv.statut === 'confirme' ? 'rgba(76,175,138,0.15)' : 'rgba(200,151,58,0.15)', color: rdv.statut === 'confirme' ? '#4CAF8A' : '#E8B55A', width: 'fit-content' }}>● {rdv.statut === 'confirme' ? 'Confirmé' : 'En attente'}</span>
              </div>
            ))}
          </div>
        </div>
