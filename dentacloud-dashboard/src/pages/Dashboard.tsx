...
const plans: Plan[] = [
  ...
  },
  {
    ...
    prix: '13 000',
    prix_usd: '99$/mois',
    desc: 'L\'offre la plus populaire',
    couleur: '#C8973A',
    populaire: true,
    features: [
      'Tout Starter inclus',
      'Assistant IA patients',
      'Confirmation automatique RDV',
  ...
  },
  {
    ...
    prix: '26 000',
    prix_usd: '199$/mois',
    desc: 'Pour les grandes cliniques',
    couleur: '#7C3AED',
    populaire: false,
    features: [
      'Tout Pro inclus',
...
function PopupPlans({ onClose }: { onClose: () => void }) {
  return (
    <div onClick={(e: React.MouseEvent<HTMLDivElement>) => e.target === e.currentTarget && onClose()} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div style={{ background: 'rgba(13,31,28,0.98)', border: '1px solid rgba(18,160,143,0.2)', borderRadius: '20px', padding: 'clamp(1.5rem, 2.5vw, 2.5rem)', width: '100%', maxWidth: 'min(900px, 95vw)', maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <p style={{ color: '#8BBDB5', fontSize: 'clamp(0.85rem, 1vw, 1rem)' }}>30 jours d'essai gratuit · Sans engagement · Annulation libre</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.2rem' }}>
          {plans.map((plan, i) => (
            <div key={i} style={{
              ...
              borderRadius: '16px', padding: 'clamp(1.5rem, 2vw, 2rem)',
              position: 'relative',
              ...
              <div style={{ fontSize: '0.75rem', fontWeight: 600, color: plan.couleur, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.5rem' }}>{plan.nom}</div>
              <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(2rem, 2.8vw, 2.8rem)', fontWeight: 700, lineHeight: 1, marginBottom: '0.2rem' }}>
                <sup style={{ fontSize: '1rem', verticalAlign: 'top', marginTop: '0.5rem', display: 'inline-block' }}>DA</sup>
                {plan.prix}
                <span style={{ fontSize: '1rem', fontWeight: 400, color: '#8BBDB5' }}>/mois</span>
              </div>
              <div style={{ fontSize: '0.72rem', color: '#8BBDB5', marginBottom: '0.3rem' }}>≈ {plan.prix_usd}</div>
              <div style={{ fontSize: '0.82rem', color: '#8BBDB5', marginBottom: '1.2rem' }}>{plan.desc}</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem', marginBottom: '1.5rem' }}>
                {plan.features.map((f, j) => (
                  <div key={j} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.82rem', color: '#8BBDB5' }}>
              ...
              <button
                onClick={() => {
                  const msg = `Bonjour DentaCloud ! Je voudrais souscrire au plan ${plan.nom} à ${plan.prix} DA/mois.`
                ...
                style={{
                  ...
                  border: plan.populaire ? 'none' : `1px solid ${plan.couleur}60`,
        ...
        <p style={{ textAlign: 'center', fontSize: '0.75rem', color: '#8BBDB5', marginTop: '1.5rem' }}>
          ✓ 30 jours d'essai gratuit · ✓ Sans carte bancaire · ✓ Annulation libre
        </p>
      </div>
    </div>
  )
...
export default function Dashboard() {
  ...
  async function fetchData(uid: string) {
    try {
      ...
      const [{ count: nbPatients, error: e1 }, { count: nbRdv, error: e2 }, { data: rdvData, error: e3 }] = responses
      if (e1) console.error('Error patients:', e1)
      if (e2) console.error('Error rdv count:', e2)
      if (e3) console.error('Error rdv list:', e3)
  ...
  const kpis = [
    ...
    { label: 'Recettes (DA)', value: '—', icon: '💰', color: '#059669', delta: 'Bientôt disponible' },
    ...
    if (joursRestants <= 0) return (
      <div style={{ background: 'linear-gradient(135deg, rgba(229,115,115,0.15), rgba(229,115,115,0.05))', border: '1px solid rgba(229,115,115,0.3)', borderRadius: '12px', padding: 'clamp(1rem, 1.3vw, 1.4rem) clamp(1.2rem, 1.8vw, 2rem)', marginBottom: '1.8rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <div style={{ fontSize: 'clamp(0.82rem, 0.95vw, 0.95rem)', color: '#8BBDB5' }}>Souscrivez maintenant pour continuer à utiliser DentaCloud</div>
        </div>
    ...
    if (joursRestants <= 7) return (
      <div style={{ background: 'linear-gradient(135deg, rgba(200,151,58,0.15), rgba(200,151,58,0.05))', border: '1px solid rgba(200,151,58,0.3)', borderRadius: '12px', padding: 'clamp(1rem, 1.3vw, 1.4rem) clamp(1.2rem, 1.8vw, 2rem)', marginBottom: '1.8rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <div style={{ fontSize: 'clamp(0.82rem, 0.95vw, 0.95rem)', color: '#8BBDB5' }}>Souscrivez avant expiration pour ne pas perdre vos données</div>
        </div>
