...
export default function Agenda() {
  ...
  async function fetchAll() {
    const [{ data: rdvData }, { data: patData }] = await Promise.all([
      supabase.from('rendez_vous').select('*, patients(nom, prenom)').order('date_heure', { ascending: true }),
  ...
  async function ajouterRdv() {
    ...
    setSaving(true)
    await supabase.from('rendez_vous').insert([{ ...form, clinique_id: null }])
  ...
  const statusColor = { confirme: { bg: 'rgba(76,175,138,0.15)', color: '#4CAF8A', label: 'Confirmé' }, en_attente: { bg: 'rgba(200,151,58,0.15)', color: '#E8B55A', label: 'En attente' }, termine: { bg: 'rgba(139,189,181,0.1)', color: '#8BBDB5', label: 'Terminé' } }
        ...
        <div style={{ ...card, padding: '1.5rem', marginBottom: '1.5rem', border: '1px solid rgba(18,160,143,0.3)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
            <div>
              ...
              <select style={{ ...input, background: '#132420' }} value={form.patient_id} onChange={e => setForm({ ...form, patient_id: e.target.value })}>
            ...
            <div>
              ...
              <input style={input} type="datetime-local" value={form.date_heure} onChange={e => setForm({ ...form, date_heure: e.target.value })} />
            </div>
            <div>
              ...
              <select style={{ ...input, background: '#132420' }} value={form.type_soin} onChange={e => setForm({ ...form, type_soin: e.target.value })}>
            ...
            <div>
              ...
              <select style={{ ...input, background: '#132420' }} value={form.statut} onChange={e => setForm({ ...form, statut: e.target.value })}>
          ...
          <div style={{ display: 'flex', gap: '0.8rem' }}>
            <button onClick={ajouterRdv} disabled={saving} style={{ background: 'linear-gradient(135deg,#0A7C6E,#12A08F)', color: '#fff', border: 'none', borderRadius: '8px', padding: '0.65rem 1.5rem', fontFamily: "'DM Sans',sans-serif", fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer' }}>
      ...
      <div style={card}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '3rem', color: '#8BBDB5' }}>
            Aucun rendez-vous — cliquez sur "+ Nouveau RDV"
          </div>
        ) : rdvs.map((rdv, i) => {
          const s = statusColor[rdv.statut] || statusColor.en_attente
          return (
            <div key={rdv.id} style={{
              display: 'grid', gridTemplateColumns: '2fr 1.2fr 1.2fr 1fr 120px', gap: '0.5rem',
              ...
              <select value={rdv.statut} onChange={e => changerStatut(rdv.id, e.target.value)} style={{
                background: s.bg, color: s.color, border: 'none', borderRadius: '100px',
                padding: '0.2rem 0.5rem', fontSize: '0.68rem', fontWeight: 500, cursor: 'pointer',
              ...
              <button onClick={() => supprimerRdv(rdv.id)} style={{
